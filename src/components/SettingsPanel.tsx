  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-3">
        <h3 className="text-sm font-medium text-neutral-900">维度权重</h3>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {DIMENSION_KEYS.map((key) => (
            <div key={key} className="flex items-center gap-2">
              <Label
                htmlFor={`weight-${key}`}
                className="w-32 shrink-0 text-xs text-neutral-700"
              >
      ...
      {/* 阈值设置 */}
      <section className="space-y-3">
        <h3 className="text-sm font-medium text-neutral-900">等级阈值</h3>
